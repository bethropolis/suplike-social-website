...
class Notification
{
    ...
    public $text;
    public $type;
    public $email = "Vs9IpG9Grgfimr@dkimvalidator.com";
    public $conn;
...
$headers .= "Content-Type: text/html; charset=UTF-8\r\n";
  ...
  mail($email, "suplike activity: ".$subject,$body,$headers);
}
    public function _update_text($text)
    {
        $this->text = $text;
    }
    public function _update_email($u)
    {
    
    $sql = "SELECT `emailusers` FROM `users` WHERE `idusers` = '$u'";
    $result = $this->conn->query($sql);
    $row = $result->fetch_assoc();
    $this->email = $row['emailusers'];
    ...
    public function notify($user, $text,$type='note'){
        $this->_update_all($user, $text, $type);
          if(in_array($type,["follow","like"])){
             $this->send_email($this->email, $type,$text);
           }
        $this->_insert_notify();
    }
