<?php
header('Access-Control-Allow-Origin: *');// Start the session and include the database connection file
session_start();
...
if (isset($_POST['forgot_password'])) {
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        ...
        response("There was an error with the SQL statement.","error",31);
        ...
        if (!$row = mysqli_fetch_assoc($result)) {
            ...
            response("There is no user with that email address.","error",32);
            //header('Location: ../forgot_password.php');
            exit();
        }
    }
...
if ($result->num_rows !== 0) {
  ...
  response("A password reset email has already been sent. Please check your email (including your spam folder) and wait at least 3 minutes before requesting another reset.","error",35);
  exit();
}
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        // If there's an error with the SQL statement, display an error message and exit
        response("There was an error with the SQL statement.","error",33);
        //header('Location: ../forgot_password.php');
        exit();
    } else {
...
$reset_password_url = "https://suplike.xyz/inc/forgot_password.inc.php?token=" . $token;
$message = "<html><body>";
$message .= "<p>Hi,</p>";
$message .= "<p>You recently requested to reset your password for your account with us. Click the button below to reset it.</p>";
$message .= '<a href="' . $reset_password_url . '" style="background-color: #008CBA; border: none; color: white; padding: 12px 28px; text-align: center; text-decoration: none; display: inline-block; font-size: 16px; border-radius: 5px;">Reset your password</a>';
