    ...
    <form class="form mx-auto co bga" action="inc/signup.inc.php" method="post" enctype="multipart/form-data">
      <div class="form-group">
        ...
        <div class="form-label-group">
          ...
          <input type="password" id="pwd" name="pwd" class="w-100 text-dark form-control" title="enter your password" placeholder="password..." pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,}" oninvalid="this.setCustomValidity('password should be 6 characters long and contain at least one number, one uppercase and one lowercase letter')" oninput="setCustomValidity('')" required>
        </div>
